...
<span id="error">
 ...
 ?>  > <span>Village here</span></span>
		</div>
	</div>

	<div class="w3ls-list">
		<div class="container">
		<h2>Village Matrimonial Profiles list</h2>
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
$sql = "SELECT * FROM user where gender!='$_SESSION[gender]'AND village='$village' AND pay_status='1' ";
$result = $conn->query($sql);
...
?>
   ...
   $sql = "SELECT * FROM user where gender!='$_SESSION[gender]'AND village='$village' ";
$result = $conn->query($sql);
