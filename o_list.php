...
<span id="error">
 ...
 ?>  > <span>Occupation here</span></span>
		</div>
	</div>
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
$sql = "SELECT * FROM user where gender!='$_SESSION[gender]'AND work='$work' AND pay_status='1'";
$result = $conn->query($sql);
...
{
$sql = "SELECT * FROM user where gender!='$_SESSION[gender]'AND work='$work' ";
$result = $conn->query($sql);
