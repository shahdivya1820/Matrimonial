...
$sql = "SELECT * from user where email='$_SESSION[email]' AND verify1='1'";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
		...
		{
			header ("Location:groom_profile.php");
		}
   ...
   header ("Location:bride_profile.php");
}
...
include("database/db.php");
			...
			$result = mysqli_query($conn,"SELECT * FROM user where gender!='$_SESSION[gender]'AND id!='$id' and verify1='1' order by RAND()  DESC limit 8 ");
    ...
    while($row = $result->fetch_assoc()) {
       ?>
			<div class="single_w3_profile">
				...
				</div>	<div id="modal01" class="modal12" onclick="this.style.display='none'">
...
<p><?php  echo  $row["gender"] ;?>,<?php  echo  $row["age"] ;?> Years,<?php  echo  $row["education"] ;?>,  <?php  echo  $row["work"] ;?>.</p>
            ...
            $count=mysqli_num_rows($query);
             ...
             ?>
				...
				{
				  ?>
					<a href="profile0.php"  >Received Requests (<?php  echo $count; ?>)</a>
					<?php	
				}
			 }
			 else {
...
?>
					...
					<div class="form_but1">
						<label class="col-sm-3 control-label1" for="Relation">Current Company Name: </label>
						<div class="col-sm-9 w3_details">
							<?php
include("database/db.php");
