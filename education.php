...
<span id="error">
 ...
 ?>  > <span>Education here</span></span>
		</div>
	</div>

	<div class="w3ls-list">
		<div class="container">
		<h2>Education Matrimonial Profiles list</h2>
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
$sql = "SELECT * FROM user where gender!='$_SESSION[gender]'AND education='$education' AND pay_status='1' ";
$result = $conn->query($sql);
...
{
  $sql = "SELECT * FROM user where gender!='$_SESSION[gender]'AND education='$education' ";
$result = $conn->query($sql);
